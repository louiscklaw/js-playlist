...
const input_fs = fs.readFileSync(
  '/home/logic/_workspace/react-playlist/admin-template/react-material-admin/public/locales/jp/translation.json', 
  {encoding:'utf-8'}
  )
...
function iter(o) {
  Object.keys(o).forEach(function (k) {
      ...
      if (typeof o[k] === 'string') {
        o[`${k}_original`] = o[k];
        let output = loremIpsum({count: o[k].split(' ').length, units: 'words'})
        o[`${k}`] = output;
      }
  });
}
...
fs.writeFileSync('output.json', JSON.stringify(input_json, null, 2), {encoding:'utf-8'})