describe('Chart.DatasetController', function () {
  it('should listen for dataset data insertions or removals', function () {
    ...
    var methods = ['_onDataPush', '_onDataPop', '_onDataShift', '_onDataSplice', '_onDataUnshift']
  ...
  it('should parse data using correct scales', function () {
    ...
    const chart = acquireChart({
      type: 'line',
      data: {
        datasets: [{ data: data1 }, { data: data2, xAxisID: 'x2', yAxisID: 'y2' }],
      },
      options: {
        scales: {
          x: {
  ...
  it('should parse using provided keys', function () {
    const chart = acquireChart({
      ...
      data: {
        datasets: [
          {
            data: [{ x: 1, data: { key: 'one', value: 20 } }, { data: { key: 'two', value: 30 } }],
          },
        ],
      },
      options: {
  ...
  describe('_resolveOptions', function () {
    it('should resove names in array notation', function () {
      ...
      expect(controller._resolveOptions(['datasetTest', 'elementTest', 'globalTest'], { type: 'line' })).toEqual({
        datasetTest: 'dataset',
        elementTest: 'element',
        globalTest: 'global',
      })
