...
import { ITooltipChartOptions, IFillerControllerDatasetOptions, ILegendChartOptions, ITitleChartOptions } from './plugins'
...
export type ILineControllerDataset<T = IScatterDataPoint> = IChartDataset<T, ILineControllerDatasetOptions & IFillerControllerDatasetOptions>
...
export interface IChartData<T = number, L = string, DS extends IChartDataset<T> = IChartDataset<T>> extends DeepPartial<IParsingOptions> {
  labels: L[]
  datasets: DS[]
}

export type IChartOptions<O = {}> = DeepPartial<
  ICoreChartOptions & ITooltipChartOptions & ILegendChartOptions & ITitleChartOptions & IChartAnimationOptions & IScaleChartOptions & IElementChartOptions & O
>

export interface IChartConfiguration<TYPE = string, T = number, L = string, DS extends IChartDataset<T> = IChartDataset<T>, O = {}> {
  type: TYPE
  data: IChartData<T, L, DS>
  options?: IChartOptions<O>
}

export type IBarControllerConfiguration<T = number, L = string> = IChartConfiguration<'bar', T, L, IBarControllerDataset<T>, IBarControllerChartOptions>
...
export type IScatterControllerConfiguration<T = IScatterDataPoint, L = string> = IChartConfiguration<
  ...
  IScatterControllerChartOptions
>
export type IBubbleControllerConfiguration<T = IBubbleDataPoint, L = string> = IChartConfiguration<'bubble', T, L, IBubbleControllerDataset<T>>
export type IPieControllerConfiguration<T = IPieDataPoint, L = string> = IChartConfiguration<'pie', T, L, IPieControllerDataset<T>, IPieControllerChartOptions>
...
export type IPolarAreaControllerConfiguration<T = number, L = string> = IChartConfiguration<
  ...
  IPolarAreaControllerChartOptions
>
export type IRadarControllerConfiguration<T = number, L = string> = IChartConfiguration<'radar', T, L, IRadarControllerDataset<T>, IRadarControllerChartOptions>

export type ConfigurationOptions<O> = O extends IChartConfiguration<unknown, unknown, unknown, infer O> ? O : never
export type ConfigurationData<O> = O extends IChartConfiguration<unknown, infer T, infer L, infer DS, unknown> ? IChartData<T, L, DS> : never
export type ConfigurationDataset<O> = O extends IChartConfiguration<unknown, unknown, unknown, infer DS, unknown> ? DS : never
