...
export const ScheduleListTable = props => {
  const {
    schedules,
    schedulesCount,
    onPageChange,
  ...
  const handleSelectAllCustomers = event => {
    setSelectedCustomers(
      event.target.checked ? schedules.map(customer => customer.id) : [],
    );
  };
  ...
  const selectedSomeCustomers =
    selectedCustomers.length > 0 && selectedCustomers.length < schedules.length;
  const selectedAllCustomers = selectedCustomers.length === schedules.length;
