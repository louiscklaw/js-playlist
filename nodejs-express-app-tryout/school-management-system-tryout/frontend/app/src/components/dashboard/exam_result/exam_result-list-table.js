...
export const ExamResultListTable = props => {
  const {
    exam_results,
    examResultsCount,
    onPageChange,
  ...
  const handleSelectAllCustomers = event => {
    setSelectedCustomers(
      event.target.checked ? exam_results.map(customer => customer.id) : [],
    );
  };
  ...
  const selectedSomeCustomers =
    ...
    selectedCustomers.length < exam_results.length;
  const selectedAllCustomers = selectedCustomers.length === exam_results.length;
