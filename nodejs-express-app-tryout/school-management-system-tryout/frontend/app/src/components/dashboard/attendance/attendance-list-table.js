...
export const AttendanceListTable = props => {
  ...
  const handleSelectAllCustomers = event => {
    setSelectedCustomers(
      event.target.checked ? attendances.map(customer => customer.id) : [],
    );
  };
  ...
  const selectedSomeCustomers =
    ...
    selectedCustomers.length < attendances.length;
  const selectedAllCustomers = selectedCustomers.length === attendances.length;
