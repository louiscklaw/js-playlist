...
export const ClassroomListTable = props => {
  const {
    classrooms,
    classroomsCount,
    onPageChange,
  ...
  const handleSelectAllCustomers = event => {
    setSelectedCustomers(
      event.target.checked ? classrooms.map(customer => customer.id) : [],
    );
  };
  ...
  const selectedSomeCustomers =
    ...
    selectedCustomers.length < classrooms.length;
  const selectedAllCustomers = selectedCustomers.length === classrooms.length;
