...
export const ExamListTable = props => {
  const {
    exams,
    examsCount,
    onPageChange,
  ...
  const handleSelectAllCustomers = event => {
    setSelectedCustomers(
      event.target.checked ? exams.map(customer => customer.id) : [],
    );
  };
  ...
  const selectedSomeCustomers =
    selectedCustomers.length > 0 && selectedCustomers.length < exams.length;
  const selectedAllCustomers = selectedCustomers.length === exams.length;
