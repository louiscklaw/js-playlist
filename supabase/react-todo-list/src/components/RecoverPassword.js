...
const RecoverPassword = ({ token, setRecoveryToken }) => {
  ...
  return (
    <div className={"w-full h-full sm:h-auto sm:w-2/5 max-w-sm p-5 bg-white shadow flex flex-col text-base"}>
      <span className={"font-sans text-2xl text-center pb-2 mb-1 border-b mx-4 align-center"}>Recover Password</span>
      <label className={"mt-3 mb-2 text-lg"} htmlFor={"email"}>
        <span className={"font-mono mr-1 text-red-400"}>*</span>Enter new password:
      </label>
      <input className={"bg-gray-100 border py-1 px-3"} type="password" ref={newPasswordRef} required />
