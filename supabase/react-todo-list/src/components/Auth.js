...
const Auth = () => {
  ...
  return (
    <div className={"w-full h-full sm:h-auto sm:w-2/5 max-w-sm p-5 bg-white shadow flex flex-col text-base"}>
      ...
      <input className={"bg-gray-100 border py-1 px-3"} type={"email"} name={"email"} ref={emailRef} required />
      ...
      <input
        ...
        required
      />
      <span className={"text-blue-600 mt-2 cursor-pointer self-end text-sm font-medium"} onClick={forgotPassword}>
        Forgot Password?
      </span>
      {!!helperText.text && (
        <div
          className={`border px-1 py-2 my-2 text-center text-sm ${
            helperText.error ? "bg-red-100 border-red-300 text-red-400" : "bg-green-100 border-green-300 text-green-500"
          }`}
        >
          {helperText.text}
        </div>
