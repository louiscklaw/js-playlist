...
function getOpenHours(day, start, end) {
  ...
  showText += ", ";
  if (parseInt(start1, 10) <= 12)
    showText = showText + start1 + start2 + getStr("AM") + " - ";
  else
    showText =
      showText + (parseInt(start1, 10) - 12) + start2 + getStr("PM") + " - ";
  if (parseInt(end1, 10) <= 12)
    showText = showText + end1 + end2 + getStr("AM");
