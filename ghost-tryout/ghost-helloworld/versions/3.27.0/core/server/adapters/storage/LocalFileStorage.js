  ...
  saveRaw(buffer, targetPath) {
    ...
    return fs
      ...
      .then(() => {
        ...
        const fullUrl = urlUtils.urlJoin('/', urlUtils.getSubdir(), urlUtils.STATIC_IMAGE_URL_PREFIX, targetPath).replace(new RegExp(`\\${path.sep}`, 'g'), '/')
    ...
    return this.getUniqueFileName(image, targetDir)
      ...
      .then(() => {
        ...
        const fullUrl = urlUtils
          .urlJoin('/', urlUtils.getSubdir(), urlUtils.STATIC_IMAGE_URL_PREFIX, path.relative(this.storagePath, targetFilename))
