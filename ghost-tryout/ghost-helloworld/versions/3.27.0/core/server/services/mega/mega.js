...
const getEmailData = async (postModel, members = []) => {
  ...
  replacements.forEach(replacement => {
    emailTmpl[replacement.format] = emailTmpl[replacement.format].replace(replacement.match, `%recipient.${replacement.id}%`)
  })
...
const addEmail = async (postModel, options) => {
  ...
  if (!existing) {
    ...
    replacements.forEach(replacement => {
      emailTmpl[replacement.format] = emailTmpl[replacement.format].replace(replacement.match, replacement.fallback || '')
    })
