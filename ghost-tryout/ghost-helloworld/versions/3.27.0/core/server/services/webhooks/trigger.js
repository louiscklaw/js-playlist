...
const webhooks = {
  getAll(event) {
    return models.Webhook.findAllByEvent(event, { context: { internal: true } })
  },
  ...
  destroy(webhook) {
    return models.Webhook.destroy({ id: webhook.id }, { context: { internal: true } }).catch(() => {
  ...
  webhooks.getAll(event).then(webhooks => {
    ...
    _.each(webhooks.models, webhook => {
      payload(webhook.get('event'), model).then(payload => {
        ...
        request(url, opts).then(response.onSuccess(webhook)).catch(response.onError(webhook))
      })
    })
  })
}
