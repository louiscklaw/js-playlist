...
function validatePassword(password, email, blogTitle) {
  ...
  const badPasswords = ['1234567890', 'qwertyuiop', 'qwertzuiop', 'asdfghjkl;', 'abcdefghij', '0987654321', '1q2w3e4r5t', '12345asdfg']
...
function validateSchema(tableName, model, options) {
  ...
  _.each(columns, function each(columnKey) {
    ...
    if (Object.prototype.hasOwnProperty.call(schema[tableName][columnKey], 'type') && schema[tableName][columnKey].type === 'bool') {
...
function validate(value, key, validations, tableName) {
  ...
  _.each(
    validations,
    function each(validationOptions, validationName) {
      let goodResult = true
      ...
      if (validator[validationName].apply(validator, validationOptions) !== goodResult) {
        ...
        if (i18n.doesTranslationKeyExist('notices.data.validation.index.validationFailedTypes.' + validationName)) {
          translation = i18n.t(
            'notices.data.validation.index.validationFailedTypes.' + validationName,
            _.merge(
              {
