...
exportFileName = function exportFileName(options) {
  ...
  return models.Settings.findOne({ key: 'title' }, _.merge({}, modelOptions, _.pick(options, 'transacting')))
...
exportTable = function exportTable(tableName, options) {
  if (EXCLUDED_TABLES.indexOf(tableName) < 0 || (options.include && _.isArray(options.include) && options.include.indexOf(tableName) !== -1)) {
...
doExport = function doExport(options) {
  ...
  return getVersionAndTables(options)
    .then(function exportAllTables(result) {
