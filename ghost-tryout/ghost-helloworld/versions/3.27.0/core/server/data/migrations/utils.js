...
function addPermissionToRole(config) {
  return createTransactionalMigration(
    async function up(connection) {
      ...
      if (!permission) {
        throw new Error(`Cannot add permission(${config.permission}) to role(${config.role}) - permission does not exist`)
      }
      ...
      if (!role) {
        throw new Error(`Cannot add permission(${config.permission}) to role(${config.role}) - role does not exist`)
      }
    ...
    async function down(connection) {
      ...
      if (!permission) {
        throw new Error(`Cannot remove permission(${config.permission}) from role(${config.role}) - permission does not exist`)
      }
      ...
      if (!role) {
        throw new Error(`Cannot remove permission(${config.permission}) from role(${config.role}) - role does not exist`)
      }
...
function addPermissionWithRoles(config, roles) {
  return combineTransactionalMigrations(addPermission(config), ...roles.map(role => addPermissionToRole({ permission: config.name, role })))
}
