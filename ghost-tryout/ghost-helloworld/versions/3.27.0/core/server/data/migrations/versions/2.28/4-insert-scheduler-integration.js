...
_private.addGhostSchedulerIntegration = options => {
  ...
  return models.Integration.findOne({ slug: fixtureIntegration.slug }, options).then(integration => {
    if (!integration) {
      return utils
        .addFixturesForModel(
          {
...
_private.removeGhostSchedulerIntegration = options => {
  ...
  return models.Integration.findOne({ slug: 'ghost-scheduler' }, options).then(integration => {
    if (!integration) {
      logging.warn(message)
      return
    }
