...
const typeGroupMapping = [
  {
    keys: ['members_public_key', 'members_private_key', 'members_email_auth_secret'],
    from: 'members',
    to: 'core',
  },
  {
...
const groupMapping = [
  {
    group: 'core',
    keys: ['db_hash', 'next_update_check', 'notifications', 'session_secret', 'theme_session_secret', 'ghost_public_key', 'ghost_private_key'],
  },
  {
    group: 'theme',
    keys: ['active_theme'],
  ...
  {
    group: 'private',
    keys: ['is_private', 'password', 'public_hash'],
  },
  {
    group: 'members',
    keys: ['default_content_visibility', 'members_subscription_settings', 'stripe_connect_integration'],
  },
  {
    group: 'portal',
    keys: ['portal_name', 'portal_button', 'portal_plans'],
  },
]
...
module.exports = {
  ...
  async up(options) {
    ...
    await Promise.map(typeGroupMapping, async typeGroupMap => {
      return await Promise.map(typeGroupMap.keys, async key => {
        ...
        return await options.transacting('settings').where('key', key).update({
    ...
    return await Promise.map(flagMapping, async flagMap => {
      ...
      return await options.transacting('settings').where('key', flagMap.key).update({
        flags: flagMap.flags,
      })
    })
  },
  ...
  async down(options) {
    ...
    await options.transacting('settings').update({
      flags: null,
    })
