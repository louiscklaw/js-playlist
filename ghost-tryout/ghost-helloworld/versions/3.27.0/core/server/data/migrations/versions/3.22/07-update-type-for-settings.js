...
const typeMapping = [
  ...
  {
    keys: ['notifications', 'navigation', 'secondary_navigation', 'slack', 'shared_views', 'portal_plans', 'stripe_plans'],
    type: 'array',
  },
  {
    keys: ['next_update_check'],
    type: 'number',
  },
  {
    keys: ['amp', 'is_private', 'members_allow_free_signup', 'portal_name', 'portal_button', 'stripe_connect_livemode'],
    type: 'boolean',
  },
  {
    keys: ['labs', 'unsplash', 'bulk_email_settings'],
    type: 'object',
  },
]
  ...
  async down(options) {
    ...
    return await Promise.map(typeMapping, async typeMap => {
      return await Promise.map(typeMap.keys, async key => {
        ...
        const groupResult = await options.transacting('settings').where('key', key).select('group')
