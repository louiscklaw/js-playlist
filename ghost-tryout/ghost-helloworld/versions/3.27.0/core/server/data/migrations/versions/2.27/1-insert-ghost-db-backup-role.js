...
_private.removeApiKeyPermissionsAndRole = options => {
  ...
  return models.Role.findOne({ name: 'DB Backup Integration' }, options).then(role => {
    if (!role) {
      logging.warn(message)
      return
    }
...
module.exports.up = options => {
  ...
  return _private.addApiKeyRole(localOptions).then(() => _private.addApiKeyPermissions(localOptions))
}
