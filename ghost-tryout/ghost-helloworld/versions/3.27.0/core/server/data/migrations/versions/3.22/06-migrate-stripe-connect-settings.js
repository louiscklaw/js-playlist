...
module.exports = {
  ...
  async up(config) {
    ...
    const stripeConnectIntegrationJSON = await knex('settings').select('value').where('key', 'stripe_connect_integration').first()
    ...
    await knex('settings').where('key', 'stripe_connect_integration').del()
  },
