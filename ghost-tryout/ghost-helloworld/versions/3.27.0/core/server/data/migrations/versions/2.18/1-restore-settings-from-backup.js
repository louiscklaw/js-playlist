...
module.exports.up = options => {
  ...
  return fs
    ...
    .then(function (files) {
      ...
      try {
        ...
        logging.warn(
          `Could not read ${path.join(
            dataPath,
            mostRecentBackup,
          )} whilst trying to ensure boolean settings are correct. Please double check your settings after this upgrade`,
        )
        return
      }
      ...
      return localOptions.transacting('migrations').then(response => {
        if (!response) {
        ...
        const relevantBackupSettings = settings
        ...
        return localOptions.transacting('settings').then(response => {
          if (!response) {
