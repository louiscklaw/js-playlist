...
_private.addSchedulerRole = options => {
  ...
  return models.Role.findOne({ name: apiKeyRole.name }, options).then(role => {
    if (!role) {
      return utils
        .addFixturesForModel(
          {
        ...
        .then(result => _private.printResult(result, message))
    }
...
_private.addPublishPermission = options => {
  ...
  return utils.addFixturesForModel(modelToAdd, options).then(result => _private.printResult(result, `Adding "publish" permissions fixtures for ${resource}s`))
}
...
_private.removeApiKeyPermissionsAndRole = options => {
  ...
  return utils
    .removeFixturesForModel(modelToRemove, options)
...
module.exports.up = options => {
  ...
  return _private.addSchedulerRole(localOptions).then(() => _private.addPublishPermission(localOptions))
}
