...
module.exports = {
  ...
  async up(config) {
    ...
    const membersSubscriptionSettingsJSON = await knex('settings').select('value').where('key', 'members_subscription_settings').first()
    ...
    await knex('settings').where('key', 'members_subscription_settings').del()
  },
