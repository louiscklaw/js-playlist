...
module.exports = {
  docName: 'posts',
  browse: {
    options: ['include', 'filter', 'fields', 'formats', 'limit', 'order', 'page', 'debug', 'absolute_urls'],
    validation: {
      options: {
        include: {
  ...
  add: {
    statusCode: 201,
    headers: {},
    options: ['include', 'formats', 'source', 'send_email_when_published'],
    validation: {
      options: {
        include: {
          values: allowedIncludes,
    ...
    query(frame) {
      return models.Post.add(frame.data.posts[0], frame.options).then(model => {
  ...
  edit: {
    ...
    async query(frame) {
      ...
      if ((model.get('status') === 'published' && model.wasChanged()) || (model.get('status') === 'draft' && model.previous('status') === 'published')) {
        this.headers.cacheInvalidate = true
      } else if ((model.get('status') === 'draft' && model.previous('status') !== 'published') || (model.get('status') === 'scheduled' && model.wasChanged())) {
