...
const listMembers = async function (options) {
  ...
  const members = await Promise.all(
    memberModels.map(async function (member) {
...
const members = {
  ...
  browse: {
    options: ['limit', 'fields', 'filter', 'order', 'debug', 'page', 'search', 'paid'],
    permissions: true,
    validation: {},
    async query(frame) {
    ...
    async query(frame) {
      ...
      try {
        ...
        const isStripeLinkingError =
          error.message && (error.message.match(/customer|plan|subscription/g) || error.context === i18n.t('errors.api.members.stripeNotConnected.context'))
  ...
  destroy: {
    ...
    async query(frame) {
      ...
      await Promise.resolve(models.Member.destroy(frame.options)).catch(models.Member.NotFoundError, () => {
        throw new errors.NotFoundError({
          message: i18n.t('errors.api.resource.resourceNotFound', {
            resource: 'Member',
          }),
      ...
      async function getTotalMembersOnDatesInRange() {
        ...
        if (isSQLite) {
          ...
          result = await db
            .knex('members')
            .select(
              db.knex.raw("DATE(CONVERT_TZ(created_at, '+00:00', ?)) AS created_at, COUNT(CONVERT_TZ(created_at, '+00:00', ?)) AS count", [
                utcOffset,
                utcOffset,
              ]),
            )
