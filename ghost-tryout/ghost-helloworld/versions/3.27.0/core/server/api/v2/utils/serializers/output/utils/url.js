...
const forPost = (id, attrs, frame) => {
  ...
  if (attrs.mobiledoc) {
    attrs.mobiledoc = urlUtils.mobiledocRelativeToAbsolute(attrs.mobiledoc, attrs.url, urlOptions)
  }

  ;['html', 'codeinjection_head', 'codeinjection_foot'].forEach(attr => {
    if (attrs[attr]) {
      attrs[attr] = urlUtils.htmlRelativeToAbsolute(attrs[attr], attrs.url, urlOptions)
    }
  })
