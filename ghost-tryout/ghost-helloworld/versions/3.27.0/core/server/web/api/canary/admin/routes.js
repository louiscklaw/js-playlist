...
module.exports = function apiRoutes() {
  ...
  router.get('/themes/:name/download', mw.authAdminApi, http(apiCanary.themes.download))

  router.post('/themes/upload', mw.authAdminApi, apiMw.upload.single('file'), apiMw.upload.validation({ type: 'themes' }), http(apiCanary.themes.upload))

  router.put('/themes/:name/activate', mw.authAdminApi, http(apiCanary.themes.activate))

  router.del('/themes/:name', mw.authAdminApi, http(apiCanary.themes.destroy))
  ...
  router.post('/db', mw.authAdminApi, apiMw.upload.single('importfile'), apiMw.upload.validation({ type: 'db' }), http(apiCanary.db.importContent))
  router.del('/db', mw.authAdminApi, http(apiCanary.db.deleteAllContent))
  router.post('/db/backup', mw.authAdminApi, http(apiCanary.db.backupContent))
  ...
  router.post('/session', shared.middlewares.brute.globalBlock, shared.middlewares.brute.userLogin, http(apiCanary.session.add))
