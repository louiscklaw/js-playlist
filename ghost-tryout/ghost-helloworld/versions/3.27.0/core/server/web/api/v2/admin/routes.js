...
module.exports = function apiRoutes() {
  ...
  router.post('/settings/routes/yaml', mw.authAdminApi, apiMw.upload.single('routes'), apiMw.upload.validation({ type: 'routes' }), http(apiv2.settings.upload))
  ...
  router.get('/themes/:name/download', mw.authAdminApi, http(apiv2.themes.download))

  router.post('/themes/upload', mw.authAdminApi, apiMw.upload.single('file'), apiMw.upload.validation({ type: 'themes' }), http(apiv2.themes.upload))

  router.put('/themes/:name/activate', mw.authAdminApi, http(apiv2.themes.activate))

  router.del('/themes/:name', mw.authAdminApi, http(apiv2.themes.destroy))
  ...
  router.post('/db', mw.authAdminApi, apiMw.upload.single('importfile'), apiMw.upload.validation({ type: 'db' }), http(apiv2.db.importContent))
  router.del('/db', mw.authAdminApi, http(apiv2.db.deleteAllContent))
  router.post('/db/backup', mw.authAdminApi, http(apiv2.db.backupContent))
  ...
  router.post('/session', shared.middlewares.brute.globalBlock, shared.middlewares.brute.userLogin, http(apiv2.session.add))
