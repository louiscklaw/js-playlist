...
ghostBookshelf.Model = ghostBookshelf.Model.extend(
  {
    ...
    fixBools: function fixBools(attrs) {
      const self = this
      _.each(attrs, function each(value, key) {
        if (Object.prototype.hasOwnProperty.call(schema.tables[self.tableName], key) && schema.tables[self.tableName][key].type === 'bool') {
        ...
        return query.then(objects => {
          ...
          _.each(withRelated, withRelatedKey => {
            ...
            props[relation.name] = (() => {
              ...
              query.innerJoin(relation.innerJoin.relation, relation.innerJoin.condition[0], relation.innerJoin.condition[1], relation.innerJoin.condition[2])

              query.whereIn(relation.whereIn, _.map(objects, 'id'))
              query.orderBy(relation.orderBy)

              return query.then(relations => {
          ...
          return Promise.props(props).then(relations => {
