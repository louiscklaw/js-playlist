...
User = ghostBookshelf.Model.extend(
  {
    ...
    onDestroying(model, options) {
      ...
      return (options.transacting || ghostBookshelf.knex)('roles_users').where('user_id', model.id).del()
    },
    ...
    onSaving: function onSaving(newPage, attr, options) {
      ...
      if (this.hasChanged('slug') || !this.get('slug')) {
        tasks.slug = (function generateSlug() {
          return ghostBookshelf.Model.generateSlug(User, self.get('slug') || self.get('name'), {
            status: 'all',
      ...
      if (self.hasChanged('password')) {
        ...
        tasks.hashPassword = (function hashPassword() {
          return security.password.hash(self.get('password')).then(function (hash) {
  ...
  },
  {
    ...
    check: function check(object) {
      ...
      return this.getByEmail(object.email)
        .then(user => {
          ...
          return self
            .isPasswordCorrect({ plainPassword: object.password, hashedPassword: user.get('password') })
            .then(() => {
    ...
    isPasswordCorrect: function isPasswordCorrect(object) {
      ...
      return security.password.compare(plainPassword, hashedPassword).then(function (matched) {
        if (matched) {
          return
        }
    ...
    transferOwnership: function transferOwnership(object, unfilteredOptions) {
      ...
      return Promise.join(ghostBookshelf.model('Role').findOne({ name: 'Owner' }), User.findOne({ id: options.context.user }, { withRelated: ['roles'] }))
        .then(results => {
          ownerRole = results[0]
          ...
          return Promise.join(ghostBookshelf.model('Role').findOne({ name: 'Administrator' }), User.findOne({ id: object.id }, { withRelated: ['roles'] }))
        })
        .then(results => {
          ...
          return Promise.join(contextUser.roles().updatePivot({ role_id: adminRole.id }), user.roles().updatePivot({ role_id: ownerRole.id }), user.id)
        })
        .then(results => {
          return Users.forge()
