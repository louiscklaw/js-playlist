...
module.exports = function (Bookshelf) {
  ...
  const Model = Bookshelf.Model.extend({
    ...
    sync: function timestamp(options) {
      ...
      if (this.tableName !== 'posts' || !self.serverData || (options.method !== 'update' && options.method !== 'patch') || !options.method) {
        return parentSync
      }
      ...
      parentSync.update = function update() {
        return originalUpdateSync.apply(this, arguments).then(response => {
          const changed = _.omit(self._changed, ['created_at', 'updated_at', 'author_id', 'id', 'published_by', 'updated_by', 'html', 'plaintext'])
          ...
          if (Object.keys(changed).length) {
            if (clientUpdatedAt.diff(serverUpdatedAt) !== 0) {
              ...
              return Promise.reject(
                new errors.UpdateCollisionError({
