...
Settings = ghostBookshelf.Model.extend(
  ...
  },
  {
    ...
    populateDefaults: async function populateDefaults(unfilteredOptions) {
      ...
      return this.findAll(options).then(function checkAllSettings(allSettings) {
        ...
        _.each(getDefaultSettings(), function forEachDefault(defaultSetting, defaultSettingKey) {
          ...
          if (isMissingFromDB) {
            ...
            insertOperations.push(ghostBookshelf.knex.table('settings').insert(_.pick(settingValues, columns)))
          }
        })
    ...
    validators: {
      async all(model) {
        ...
        const validationErrors = validation.validate(model.get('value'), model.get('key'), settingDefault.validations, 'settings')
