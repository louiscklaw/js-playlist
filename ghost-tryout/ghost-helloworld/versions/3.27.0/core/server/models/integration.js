...
const Integration = ghostBookshelf.Model.extend(
  {
    ...
    add(data, options) {
      const addIntegration = () => {
        return ghostBookshelf.Model.add.call(this, data, options).then(({ id }) => {
    ...
    edit(data, options) {
      const editIntegration = () => {
        return ghostBookshelf.Model.edit.call(this, data, options).then(({ id }) => {
          return this.findOne({ id }, options)
        })
      }
    ...
    onSaving(integration, attrs, options) {
      ...
      if (this.hasChanged('slug') || !this.get('slug')) {
        ...
        return ghostBookshelf.Model.generateSlug(Integration, this.get('slug') || this.get('name'), { transacting: options.transacting }).then(slug => {
          this.set({ slug })
        })
      }
    },
