...
module.exports = {
  ...
  init: function initThemes() {
    ...
    return themeLoader
      ...
      .then(function activeThemeHasLoaded(theme) {
        // Validate
        return validate.check(theme).then(function validationSuccess(checkedTheme) {
          if (!validate.canActivate(checkedTheme)) {
            const checkError = new errors.ThemeValidationError({
              ...
              errorDetails: Object.assign(_.pick(checkedTheme, ['checkedVersion', 'name', 'path', 'version']), {
            ...
            if (checkedTheme.results.error.length) {
              logging.warn(
                new errors.ThemeValidationError({
  ...
  activate: function (themeName) {
    ...
    return validate.checkSafe(loadedTheme).then(checkedTheme => {
