...
loadAllThemes = function loadAllThemes() {
  return packageJSON.read.all(config.getContentPath('themes')).then(function updateThemeList(themes) {
...
loadOneTheme = function loadOneTheme(themeName) {
  return packageJSON.read.one(config.getContentPath('themes'), themeName).then(function (readThemes) {
