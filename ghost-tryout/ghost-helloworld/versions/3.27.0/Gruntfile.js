...
const configureGrunt = function (grunt) {
  ...
  const cfg = {
    ...
    watch: grunt.option('no-server-watch')
      ? { files: [] }
      : {
          livereload: {
            files: ['content/themes/casper/assets/css/*.css', 'content/themes/casper/assets/js/*.js'],
            options: {
              livereload: true,
              interval: 500,
            },
    ...
    mochacli: {
      ...
      acceptance: {
        src: ['test/api-acceptance/**/*_spec.js', 'test/frontend-acceptance/**/*_spec.js'],
      },

      regression: {
        src: ['test/regression/**/*_spec.js'],
      },
    ...
    bgShell: {
      client: {
        ...
        stdout: function (chunk) {
          ...
          const filter = grunt.option('client')
            ? false
            : [/> ghost-admin/, /^Livereload/, /^Serving on/].some(function (regexp) {
        ...
        stderr: function (chunk) {
          const skipFilter = grunt.option('client')
            ? false
            : [/- building/].some(function (regexp) {
                return regexp.test(chunk)
              })

          const errorFilter = grunt.option('client')
            ? false
  ...
  grunt.registerTask('help', 'Outputs help information if you type `grunt help` instead of `grunt --help`', function () {
  ...
  grunt.registerTask('test-all', 'Run all server tests', ['test-acceptance', 'test-unit', 'test-regression'])

  // ### Lint
  //
  // `grunt lint` will run the linter
  grunt.registerTask('lint', 'Run the code style checks for server & tests', ['shell:lint'])

  // ### test-setup *(utility)(
  // `grunt test-setup` will run all the setup tasks required for running tests
  grunt.registerTask('test-setup', 'Setup ready to run tests', ['knex-migrator', 'clean:test', 'setTestEnv', 'stubClientFiles'])
  ...
  grunt.registerTask('test-regression', 'Run regression tests.', ['test-setup', 'mochacli:regression'])

  grunt.registerTask('test-acceptance', 'Run acceptance tests', ['test-setup', 'mochacli:acceptance'])
  ...
  grunt.registerTask('init', 'Prepare the project for development', ['update_submodules:pinned', 'subgrunt:init', 'clean:tmp', 'default'])
  ...
  grunt.registerTask('prod', 'Build JS & templates for production', ['subgrunt:prod', 'uglify:prod', 'postcss:prod'])
  ...
  grunt.registerTask('master', 'Update your current working folder to latest master.', ['shell:master', 'subgrunt:init'])
